---
name: "source-command-plan-feature"
description: "Research and create a comprehensive implementation plan for a feature"
---

# source-command-plan-feature

Use this skill when the user asks to run the migrated source command `plan-feature`. Original argument hint: `<feature-description>`.

## Command Template


# Plan Feature: {user-provided arguments}
...
Find similar implementations in the codebase. Extract naming conventions, error handling patterns, logging patterns, import styles. Check AGENTS.md for project rules.
...
Save to: `.codex/plans/{kebab-case-name}.md`
...
The adversary receives: the plan file, `AGENTS.md`, and `git log --oneline -15` — nothing else. It does NOT see the planning conversation, Phase 1–4 research, or any prior reasoning.

### All Tiers: Parallel Adversarial Streams

Run two independent adversaries **in parallel** (not sequentially) at every tier:

**Stream A — Fresh Claude opus 4.7 adaptive sub-agent.** Spawn via `Task` tool with `model: "opus"`:
...
**Stream B — Codex GPT-5.5 xhigh task** (runs in background via `Bash` with `run_in_background: true`). Always `--effort xhigh`:
...
PoetiQ's core insight: iterative refinement beats single-pass reasoning. Refinement runs at every tier — Tier 1: 1 cycle, Tier 2: 1 cycle, Tier 3: up to 2 cycles.
...
Tier 1 and Tier 2 run a single cycle; skip Cycle 2.
...
2. **Write criteria** — each must be:
   ...
   - Assigned a threshold (8 minimum, 9 for security/data integrity)
