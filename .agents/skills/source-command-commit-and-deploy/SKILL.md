---
name: "source-command-commit-and-deploy"
description: "Build, test, commit by feature, and deploy — adapts to any project's toolchain"
---

# source-command-commit-and-deploy

Use this skill when the user asks to run the migrated source command `commit-and-deploy`.
...
<!-- CUSTOMIZE: Add project-specific validation steps here.
     Examples:
       - maxDuration/timeout limit checks for serverless platforms
       - Pipeline pattern validation scripts
       - Docker builds for containerized services
       - Schema drift checks
     Remove this comment block once customized.
-->
...
If any AI layer files changed (AGENTS.md, .codex/rules/, .agents/skills/source-command-), add a `Context:` section to the commit body explaining what changed and why.
...
<!-- CUSTOMIZE: Replace with your project's deploy mechanism.
     Examples:
       - git push origin main (auto-deploys via Vercel/Netlify/Railway)
       - gh workflow run deploy.yml
       - ./scripts/deploy.sh
       - kubectl apply -f k8s/
     Note expected deploy times and any multi-service deploy order.
     Remove this comment block once customized.
-->
