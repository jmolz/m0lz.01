...
| `tests/publish-crosspost.test.ts` (33 tests) | Dev.to probe-then-create + Medium + Substack paste | `mapDevToTags` lowercases, strips non-alphanumerics for Dev.to alphanumeric-only tags, caps at 4, drops empty, maps dashed source tags like `developer-tools` → `developertools`; `crosspostToDevTo` skips when `DEVTO_API_KEY` missing or empty; builds POST with api-key header, canonical_url, `published:false`, tags, description, optional `main_image` for safe draft-relative or absolute cover URLs; throws on unsafe cover paths; throws on 422 with context; throws on 503+; **probe-then-converge: GET `/api/articles/me/all` before POST, PUTs existing unpublished canonical match, rejects duplicate canonical matches, rejects published or unknown-state canonical matches, normalizes trailing slash, probe failure throws, paginates beyond page 1, stops on short page — Codex Pass 2/3 + Dev.to cover regressions**; `generateMediumPaste` writes file with H1 title, description, Medium featured image before body, canonical footer, creates nested `socialDir/slug`; `generateSubstackPaste` writes H1 title + H2 subtitle with Substack preview image before body and keeps legacy header fields readable; both apply mdxToMarkdown preserving code blocks |
