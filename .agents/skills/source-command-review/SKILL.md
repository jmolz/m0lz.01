...
| `tests/publish-crosspost.test.ts` (36 tests) | Dev.to probe-then-create + Medium + Substack paste | `mapDevToTags` lowercases, strips non-alphanumerics for Dev.to alphanumeric-only tags, caps at 4, drops empty, maps dashed source tags like `developer-tools` → `developertools`; `crosspostToDevTo` skips when `DEVTO_API_KEY` missing or empty; builds POST with api-key header, canonical_url, `published:false`, tags, description, optional `main_image` for safe draft-relative or absolute cover URLs; throws on unsafe cover paths; throws on 422 with context; throws on 503+; **probe-then-converge: GET `/api/articles/me/all` before POST, PUTs existing unpublished canonical match, rejects duplicate canonical matches, rejects published or unknown-state canonical matches, normalizes trailing slash, probe failure throws, paginates beyond page 1, stops on short page — Codex Pass 2/3 + Dev.to cover regressions**; `generateMediumPaste` writes file with H1 title, description, Medium featured image before body, canonical footer, creates nested `socialDir/slug`, and keeps image-backed MDX charts copyable as Markdown image links; `generateSubstackPaste` writes H1 title + capped H2 subtitle with Substack preview image before body, keeps legacy header fields readable, and keeps image-backed MDX charts copyable as Markdown image links; both apply mdxToMarkdown preserving code blocks |
| `tests/platform-images.test.ts` (21 tests) | Deterministic platform image generation | `ensurePlatformImages` emits exact Dev.to 1000x420 PNG, Medium 1200x675 PNG, and Substack preview 1200x630 PNG from the shared fallback SVG article-card framework; upgrades legacy generated `devto-cover.webp` defaults while preserving custom WebP covers; refreshes generator-owned defaults in draft phase but keeps them read-only during publish verification; validates and preserves explicit platform assets including URLs; rejects unsafe paths before writes/receipts/frontmatter rewrites; keeps fallback labels config-derived; throws on wrong dimensions; proves stable filenames with no numbered duplicates |
...
| `tests/distribution-kit.test.ts` (9 tests) | Durable distribution-kit generation | Metadata precedence by source mode (publish/update/backfill); hub frontmatter wins for backfill; update cycle metadata is carried without invented summaries; prompt-only mode writes text/prompt/manifest and never calls a provider; generated mode records GPT Image 2 model/size/quality/mode/fixed PNG format, prompt/input/text/image hashes, byte size, dimensions, and reuses a matching manifest byte-for-byte; `loadDistributionKit` refuses tampered text and image artifacts whose bytes no longer match manifest hashes; `OpenAIImageProvider` omits unsupported `response_format`; required mode fails without `OPENAI_API_KEY` before writing artifacts; product IDs like `m0lz.01` are not truncated to `m0lz.` |
| `tests/publish-distribution-kit.test.ts` (10 tests) | Pre-site generation + strict site persistence | `createSitePR` required-image mode fails before checkout/copy/commit/push/PR when `OPENAI_API_KEY` is missing; the real `social-text` registry step persists an existing distribution kit plus optional `assets/linkedin-feed.png` without provider/fetch calls; `persistDistributionKitToSite` stages only `content/posts/{slug}/distribution/` and optional `assets/linkedin-feed.png`, commits `chore(distribution): {slug}`, enforces origin and staged-index guards, refuses unrelated dirty site repo state and unexpected ahead commits before staging new distribution changes, returns exact no-op when current, pushes an exact crash-replay ahead commit, and refuses wrong-subject or wrong-file ahead commits |
...
Expected baseline: **0 TypeScript errors, 1077 tests passing across 78 suites, clean build**. v0.3 dogfood-hardening added +43 tests across +5 new suites — `research-project-linking` (15), `publish-origin-divergence` (6), `publish-seed-on-start` (4), `preview-gate-urls` (9), `draft-regenerate-frontmatter` (9) — plus +8 tests in `draft-frontmatter.test.ts` for companion_repo resolution. Platform article images added Dev.to/Medium/Substack-preview coverage and expanded draft/crosspost coverage. Durable distribution kits add `distribution-kit.test.ts`, `publish-distribution-kit.test.ts`, and CLI/config/plan-file coverage. Benchmark phase-gates, publish artifact verification, review-inventory tripwires, Substack subtitle caps, and copyable chart paste output bring the current suite to 1077/78; any drift from this baseline is a signal to investigate before merging.
...
Phase 6 — Publish:
  ...
  - Dev.to probe-then-create + Medium + Substack paste (33 tests): ✓ / ✗
  - Deterministic platform image generation (21 tests): ✓ / ✗
  - Social text: LinkedIn + Hacker News (14 tests): ✓ / ✗
  - Durable distribution-kit generation (9 tests): ✓ / ✗
  - Pre-site generation + strict distribution persistence (10 tests): ✓ / ✗
  - Research page generator (14 tests): ✓ / ✗
  - Companion repo probe-then-create + origin-URL guardrail (21 tests): ✓ / ✗
  - Publish CLI handlers + ctx.urls hydration + distribution-kit backfill (16 tests): ✓ / ✗
...
Full Suite: X passing, Y failing  (baseline: 1040 passing across 76 suites)
