...
| `tests/publish-table-assets.test.ts` (10 tests) | Portable Markdown table assets | Detects Markdown pipe tables outside fenced code; preserves fenced tables, malformed tables, blockquotes, and lists; renders deterministic `assets/portable-table-<hash>.png` files at Medium-compatible width; wraps long cells and long tokens inside cell bounds; supports public URL vs upload-placeholder reference policies; marks over-wide tables explicitly; reuses unchanged images; removes stale generator-owned table PNGs |
...
Expected baseline: **0 TypeScript errors, 1117 tests passing across 79 suites, clean build**. v0.3 dogfood-hardening added +43 tests across +5 new suites — `research-project-linking` (15), `publish-origin-divergence` (6), `publish-seed-on-start` (4), `preview-gate-urls` (9), `draft-regenerate-frontmatter` (9) — plus +8 tests in `draft-frontmatter.test.ts` for companion_repo resolution. Platform article images added Dev.to/Medium/Substack-preview coverage and expanded draft/crosspost coverage. Durable distribution kits add `distribution-kit.test.ts`, `publish-distribution-kit.test.ts`, `publish-table-assets.test.ts`, and CLI/config/plan-file coverage. Benchmark phase-gates, publish artifact verification, review-inventory tripwires, Substack subtitle caps, copyable chart paste output, portable table image assets, complete-bundle backfill, disabled-channel cleanup, paste-step table verification, stale deletion crash-replay coverage, Medium/Substack upload-checklist handoff, site/direct-push cleanup staging, and mirrored Claude review inventory coverage bring the current suite to 1117/79; any drift from this baseline is a signal to investigate before merging.
...
Full Suite: X passing, Y failing  (baseline: 1117 passing across 79 suites)
