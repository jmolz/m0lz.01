...
| `tests/publish-crosspost.test.ts` (38 tests) | Dev.to probe-then-create + Medium + Substack paste | `mapDevToTags` lowercases, strips non-alphanumerics for Dev.to alphanumeric-only tags, caps at 4, drops empty, maps dashed source tags like `developer-tools` -> `developertools`; `crosspostToDevTo` skips without `DEVTO_API_KEY`; builds POST with api-key header, canonical URL, unpublished status, tags, description, optional safe cover image; throws on unsafe covers and platform errors; probe-then-converge GETs `/api/articles/me/all`, PUTs existing unpublished canonical matches, rejects duplicate/published/unknown matches, normalizes trailing slash, fails closed on probe errors, paginates; Medium/Substack paste keeps image-backed MDX charts copyable, preserves fenced code blocks, caps Substack subtitles, replaces raw Markdown tables outside fences with upload-checklist placeholders, and writes platform-specific table upload checklists with canonical URL, local paths, reference-only public URLs, alt text, and captions |
...
| `tests/publish-social.test.ts` (14 tests) | LinkedIn + Hacker News text generation | `containsEmoji` detects U+1F300-1FAFF and U+2600-27BF ranges; `generateLinkedIn` fills template with title, description, canonical_url, image/prompt line, alt text, hashtags from tags; throws if output contains emojis; includes timing recommendation when `config.social.timing_recommendations=true`; writes to `{socialDir}/{slug}/linkedin.md`; `generateHackerNews` title ≤80 chars (truncates with ellipsis); `Show HN:` prefix ONLY for `project-launch`, no prefix for `technical-deep-dive` / `analysis-opinion`; includes first-comment, canonical_url, repo link only when real; throws on emoji; writes `hackernews.md`; creates output dir |
| `tests/distribution-kit.test.ts` (19 tests) | Durable distribution-kit generation | Metadata precedence by source mode (publish/update/backfill); hub MDX/frontmatter wins for published backfill; update cycle metadata is carried without invented summaries; prompt-only writes text/prompt/manifest without provider calls; Medium/Substack paste files, upload checklists, and portable table assets are manifest-tracked and hash-verified; social-disabled configs still emit publication paste artifacts when cross-post platforms are enabled; disabled platforms stay null per individual destination; generated mode records GPT Image 2 provenance and reuses matching manifests byte-for-byte; `loadDistributionKit` refuses tampered text/checklist/image/table bytes, unsafe manifest paths, symlinks, and unsafe reused manifests; required mode fails without `OPENAI_API_KEY`; product IDs like `m0lz.01` are not truncated |
| `tests/publish-distribution-kit.test.ts` (25 tests) | Pre-site generation + strict site persistence | `createSitePR` required-image mode fails before checkout/copy/commit/push/PR when `OPENAI_API_KEY` is missing; the real `social-text` registry step persists an existing manifest-derived kit including Medium/Substack paste files, upload checklists, table assets, and optional `assets/linkedin-feed.png` without provider/fetch calls; paste steps verify existing manifest artifacts without regenerating paste/table assets; disabled paste steps skip without requiring bundle files; `persistDistributionKitToSite` stages only owned distribution paths and exact optional assets, commits `chore(distribution): {slug}`, removes tracked stale generator-owned text, LinkedIn image, and portable-table artifacts when channels/images/tables are disabled or regenerated, enforces origin and staged-index guards, refuses unrelated dirty site state, conflicting reviewed artifacts, and unexpected ahead commits before staging, returns exact no-op when current, pushes exact crash-replay ahead commits including already-committed stale artifact deletions, and refuses wrong-subject/wrong-file ahead commits |
| `tests/publish-research-page.test.ts` (14 tests) | Research page MDX generator | Generates from research doc + benchmark results; includes thesis, key findings, bibliography; cross-links to post canonical URL and companion repo URL; skips for `analysis-opinion` when research doc absent with clear reason; generates for `analysis-opinion` WHEN research doc present; fills template placeholders (`{{title}}`, `{{thesis}}`, etc.); **reads description from draft MDX frontmatter, not from posts table (schema has no description column)**; idempotent (second run overwrites consistently); throws when template file missing; throws when post not in DB |
...
Expected baseline: **0 TypeScript errors, 1118 tests passing across 79 suites, clean build**. v0.3 dogfood-hardening added +43 tests across +5 new suites — `research-project-linking` (15), `publish-origin-divergence` (6), `publish-seed-on-start` (4), `preview-gate-urls` (9), `draft-regenerate-frontmatter` (9) — plus +8 tests in `draft-frontmatter.test.ts` for companion_repo resolution. Platform article images added Dev.to/Medium/Substack-preview coverage and expanded draft/crosspost coverage. Durable distribution kits add `distribution-kit.test.ts`, `publish-distribution-kit.test.ts`, `publish-table-assets.test.ts`, and CLI/config/plan-file coverage. Benchmark phase-gates, publish artifact verification, review-inventory tripwires, Substack subtitle caps, copyable chart paste output, portable table image assets, complete-bundle backfill, disabled-channel cleanup, paste-step table verification, stale deletion crash-replay coverage, Medium/Substack upload-checklist handoff, site/direct-push cleanup staging, mirrored Claude review inventory coverage, and readable row-card table images with snake_case preservation bring the current suite to 1118/79; any drift from this baseline is a signal to investigate before merging.
...
Phase 6 — Publish:
  ...
  - Portable table asset rendering (10 tests): ✓ / ✗
  - Site PR + preview gate + dirty-state + rename/copy guardrail (21 tests): ✓ / ✗
  - Dev.to probe-then-create + Medium + Substack paste (38 tests): ✓ / ✗
  - Deterministic platform image generation (22 tests): ✓ / ✗
  - Social text: LinkedIn + Hacker News (14 tests): ✓ / ✗
  - Durable distribution-kit generation (19 tests): ✓ / ✗
  - Pre-site generation + strict distribution persistence (25 tests): ✓ / ✗
  - Research page generator (14 tests): ✓ / ✗
...
Full Suite: X passing, Y failing  (baseline: 1118 passing across 79 suites)
