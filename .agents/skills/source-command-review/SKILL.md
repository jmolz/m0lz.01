...
| `tests/publish-table-assets.test.ts` (6 tests) | Portable Markdown table assets | Detects Markdown pipe tables outside fenced code; preserves fenced tables, malformed tables, blockquotes, and lists; renders deterministic `assets/portable-table-<hash>.png` files; reuses unchanged images; removes stale generator-owned table PNGs |
...
| `tests/publish-crosspost.test.ts` (38 tests) | Dev.to probe-then-create + Medium + Substack paste | `mapDevToTags` lowercases, strips non-alphanumerics for Dev.to alphanumeric-only tags, caps at 4, drops empty, maps dashed source tags like `developer-tools` -> `developertools`; `crosspostToDevTo` skips without `DEVTO_API_KEY`; builds POST with api-key header, canonical URL, unpublished status, tags, description, optional safe cover image; throws on unsafe covers and platform errors; probe-then-converge GETs `/api/articles/me/all`, PUTs existing unpublished canonical matches, rejects duplicate/published/unknown matches, normalizes trailing slash, fails closed on probe errors, paginates; Medium/Substack paste keeps image-backed MDX charts copyable, preserves fenced code blocks, caps Substack subtitles, and replaces raw Markdown tables outside fences with manifest-backed portable table image links |
...
Phase 6 — Publish:
  ...
  - Portable table asset rendering (6 tests): ✓ / ✗
...
Full Suite: X passing, Y failing  (baseline: 1112 passing across 79 suites)
