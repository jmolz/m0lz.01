...
| `tests/distribution-kit.test.ts` (8 tests) | Durable distribution-kit generation | Metadata precedence by source mode (publish/update/backfill); hub frontmatter wins for backfill; update cycle metadata is carried without invented summaries; prompt-only mode writes text/prompt/manifest and never calls a provider; generated mode records GPT Image 2 model/size/quality/mode/fixed PNG format, prompt/input/text/image hashes, byte size, dimensions, and reuses a matching manifest byte-for-byte; `loadDistributionKit` refuses tampered text and image artifacts whose bytes no longer match manifest hashes; required mode fails without `OPENAI_API_KEY` before writing artifacts; product IDs like `m0lz.01` are not truncated to `m0lz.` |
...
Phase 6 — Publish:
  ...
  - Durable distribution-kit generation (8 tests): ✓ / ✗
