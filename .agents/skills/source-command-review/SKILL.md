...
Before starting the standard review, check if the most recent contract-bearing plan has a contract:

```bash
# Find the most recently modified plan file that actually contains a contract.
# Evidence/proof notes may also live in .codex/plans/ and must not mask the feature plan.
for f in $(ls -t .codex/plans/*.md 2>/dev/null); do
...
| `tests/publish-convert.test.ts` (15 tests) | MDX → Markdown converter | Strips YAML frontmatter; removes `import` statements outside fences; converts image-backed JSX chart/figure components to copyable Markdown images before JSX stripping; preserves text children of block JSX; resolves relative `./assets/x` and `assets/x` to absolute `{base}/writing/{slug}/assets/x`; **preserves code blocks verbatim — no JSX stripping / no URL resolution inside ``` fences, including nested backtick fences (```` vs ```)**; preserves tables, headings, lists; collapses 3+ blank lines to 2; passthrough for plain Markdown; handles empty input |
| `tests/publish-table-assets.test.ts` (11 tests) | Portable Markdown table assets | Detects Markdown pipe tables outside fenced code; preserves fenced tables, malformed tables, blockquotes, and lists; renders deterministic `assets/portable-table-<hash>.png` files as readable row-card images at Medium-compatible width; wraps long cells and long tokens inside cell bounds; preserves snake_case tokens; supports public URL vs upload-placeholder reference policies; marks over-wide tables explicitly; reuses unchanged images; removes stale generator-owned table PNGs |
| `tests/publish-site.test.ts` (21 tests) | `createSitePR` + `checkPreviewGate` (site repo PR management) | Copies draft MDX to `{siteRepoPath}/{config.site.content_dir}/{slug}/index.mdx`; copies assets directory recursively; copies research page to `config.site.research_dir`; copies complete publication bundle artifacts (`distribution/manifest.json`, social text, Medium/Substack paste files, portable table images) before preview; creates branch `post/{slug}` from main; idempotent on existing branch; commits with `feat(post):`; pushes/opens PR; writes `pr-number.txt`; `checkPreviewGate` polls `gh pr view`; all subprocess via `execFileSync` argv arrays; dirty-state guardrail tolerates only exact pipeline-owned paths; stages only exact current output paths plus tracked stale owned deletions; recomputes branch-dependent stale deletion staging after existing-branch checkout; rename/copy parsing rejects escapes |
| `tests/publish-crosspost.test.ts` (40 tests) | Dev.to probe-then-create + Medium + Substack paste | `mapDevToTags` lowercases, strips non-alphanumerics for Dev.to alphanumeric-only tags, caps at 4, drops empty, maps dashed source tags like `developer-tools` -> `developertools`; `crosspostToDevTo` skips without `DEVTO_API_KEY`; builds POST with api-key header, canonical URL, unpublished status, tags, description, optional safe cover image; throws on unsafe covers and platform errors; probe-then-converge GETs `/api/articles/me/all`, PUTs existing unpublished canonical matches, rejects duplicate/published/unknown matches, normalizes trailing slash, fails closed on probe errors, paginates; Medium/Substack paste keeps image-backed MDX charts copyable, preserves fenced code blocks, naturally fits Substack subtitles without ellipses, replaces already-clipped subtitles with safe fallback copy, and fails when no safe fit exists, replaces raw Markdown tables outside fences with upload-checklist placeholders, and writes platform-specific table upload checklists with canonical URL, local paths, reference-only public URLs, alt text, and captions |
| `tests/platform-images.test.ts` (22 tests) | Deterministic platform image generation | `ensurePlatformImages` emits exact Dev.to 1000x420 PNG, Medium 1200x675 PNG, and Substack preview 1200x630 PNG from the shared fallback SVG article-card framework; upgrades legacy generated `devto-cover.webp` defaults while preserving custom WebP covers; refreshes generator-owned defaults in draft phase but keeps them read-only during publish verification; validates and preserves explicit platform assets including URLs; rejects unsafe paths before writes/receipts/frontmatter rewrites; keeps fallback labels config-derived; throws on wrong dimensions; proves stable filenames with no numbered duplicates |
| `tests/publish-social.test.ts` (17 tests) | LinkedIn + Hacker News text generation | `containsEmoji` detects U+1F300-1FAFF and U+2600-27BF ranges; `generateLinkedIn` fills a public-only pinned shape with hook, body, canonical URL, hashtags, and optional timing; rejects internal image prompt/alt/path labels, emojis, and abrupt ellipsis clipping; replaces already-clipped descriptions with safe fallback copy; writes to `{socialDir}/{slug}/linkedin.md`; `generateHackerNews` title ≤80 chars (title truncation only), `Show HN:` prefix ONLY for `project-launch`, no prefix for `technical-deep-dive` / `analysis-opinion`; fits first-comment descriptions without abrupt ellipses; includes canonical_url and repo link only when real; throws on emoji; writes `hackernews.md`; creates output dir |
| `tests/distribution-kit.test.ts` (28 tests) | Durable distribution-kit generation | Metadata precedence by source mode (publish/update/backfill); hub MDX/frontmatter wins for published backfill; update cycle metadata is carried without invented summaries; prompt-only writes prompt/manifest without provider calls while public LinkedIn copy stays artifact-free and legacy OpenAI-null-image manifests still load; local-card writes a 1200x1200 `assets/linkedin-feed.png` without provider calls, records local-card provenance, and refuses symlink output paths; local-card SVG output escapes and bounds long text while avoiding desk/workspace motifs; prompt quality guards reject desk/workspace motifs without rejecting metadata that mentions release notes or laptops; Medium/Substack paste files, upload checklists, and portable table assets are manifest-tracked and hash-verified; social-disabled configs still emit publication paste artifacts when cross-post platforms are enabled; disabled platforms stay null per individual destination; generated mode records GPT Image 2 provenance and reuses matching manifests byte-for-byte; manifest reuse compares current expected artifact hashes instead of trusting rewritten manifest hashes; `loadDistributionKit` refuses tampered text/checklist/image/table bytes, unsafe manifest paths, symlinks, and unsafe reused manifests; required mode fails without `OPENAI_API_KEY`; product IDs like `m0lz.01` are not truncated and lead-sentence helpers throw instead of hard-clipping |
| `tests/publish-distribution-kit.test.ts` (27 tests) | Pre-site generation + strict site persistence | `createSitePR` required-image mode and no-safe-fit Substack subtitles fail before checkout/copy/commit/push/PR; the real `social-text` registry step reuses a valid manifest-derived kit including Medium/Substack paste files, upload checklists, table assets, and optional `assets/linkedin-feed.png` without provider/fetch calls, and regenerates self-referential tampered text before site persistence; paste steps verify existing manifest artifacts without regenerating paste/table assets; disabled paste steps skip without requiring bundle files; `persistDistributionKitToSite` stages only owned distribution paths and exact optional assets, commits `chore(distribution): {slug}`, removes tracked stale generator-owned text, LinkedIn image, and portable-table artifacts when channels/images/tables are disabled or regenerated, enforces origin and staged-index guards, refuses unrelated dirty site state, conflicting reviewed artifacts, and unexpected ahead commits before staging, returns exact no-op when current, pushes exact crash-replay ahead commits including already-committed stale artifact deletions, and refuses wrong-subject/wrong-file ahead commits |
| `tests/publish-research-page.test.ts` (15 tests) | Research page MDX generator | Generates from research doc + benchmark results; includes thesis, key findings, bibliography; cross-links to post canonical URL and companion repo URL; skips for `analysis-opinion` when research doc absent with clear reason; generates for `analysis-opinion` WHEN research doc present; fills template placeholders (`{{title}}`, `{{thesis}}`, etc.); **reads description from draft MDX frontmatter, not from posts table (schema has no description column)**; idempotent (second run overwrites consistently); throws when template file missing; throws when post not in DB |
...
| `tests/guidance-docs.test.ts` (8 tests) | Command/rule guidance tripwires | Review command docs must describe Dev.to tags as alphanumeric-only and never preserve punctuation; evaluate/plan-feature docs must treat tenant-policy data-exfiltration denials as `policy-blocked` with no Responses API fallback; self-heal docs must select contract-bearing source plans before proof/evidence artifacts; Codex source-command review docs must reference the real `.claude-plugin` surface, not absent Codex plugin paths; `.claude` and `.codex` drafting rules must agree on the `PostFrontmatter` field list including `devto_main_image` and lifecycle fields; publish guidance captures complete-bundle and portable-table invariants |
| `tests/review-inventory.test.ts` (3 tests) | Review regression inventory tripwire | Asserts `.agents/skills/source-command-review/SKILL.md`, `.claude/commands/review.md`, and `.windsurf/workflows/review.md` mention every current `tests/*.test.ts` file, so newly added tripwire tests cannot be silently omitted from the cumulative review command or its mirrored review workflow. |
...
| `tests/research-set-section.test.ts` (13 tests) | `blog research set-section` CLI + library | `setResearchSection(researchDir, slug, section, content)` replaces one of the 7 keyed sections (thesis, findings, sources_list, data_points, open_questions, benchmark_targets, repo_scope) while preserving frontmatter and all other sections; invalid section key rejected with enumerated valid keys in error; path-traversal slug rejected at library layer via `validateSlug`; missing-doc throw with actionable "run blog research init first" hint; overwrite semantics (replace, not append); populating all 7 sections satisfies the document-gate half of finalize. CLI handler: `--content` inline prose path; `--from-file <path>` reads file and injects; mutual-exclusion (both flags → exitCode=1); empty-content rejected; non-research phase rejected; missing `--from-file` target rejected; path-traversal slug rejected at CLI boundary before DB open. Enables the skill-driven research-authoring path (hash-bound section prose travels through the plan) so the skill never needs Write/Edit scope to complete the finalize gate. |
| `tests/voice-parity.test.ts` (4 tests) | Voice rules byte-parity between dev rule and plugin skill | `.claude/rules/voice.md` (dev-facing, path-scoped auto-load) and `.claude-plugin/skills/blog/VOICE.md` (shipped with plugin tarball, loaded by `/blog` skill when composing set-section prose) must stay byte-identical below the frontmatter boundary. `stripFrontmatter` regex extracts body after closing `---\n`; parity assertion catches any drift between the two sources. Dev rule must have `paths:` frontmatter; plugin file must have NO frontmatter (plugin skills don't path-scope); plugin file must not contain `jmolz | m0lz.dev | @molz | DEVTO_API_KEY=` identity strings (skill-content hygiene, duplicated here so a failure points at the right file). Prevents "voice rule updated in one place, other location drifts" silent divergence. |
...
- `src/cli/research.ts` — `runResearchInit`/`runResearchAddSource`/`runResearchShow`/`runResearchFinalize`/`runResearchSetSection`, slug validation, phase boundary
...
- `src/core/research/document.ts` — `writeResearchDocument`, `readResearchDocument`, `validateResearchDocument`, `setResearchSection` + `SECTION_KEYS` const, `validateSlug`, path traversal guard
- `src/cli/benchmark.ts` — `runBenchmarkInit`/`runBenchmarkEnv`/`runBenchmarkRun`/`runBenchmarkShow`/`runBenchmarkSkip`/`runBenchmarkComplete`/`runBenchmarkRepair`, `BenchmarkPaths`, slug validation, phase boundary
- `src/core/benchmark/environment.ts` — `captureEnvironment`, `formatEnvironmentMarkdown`
- `src/core/benchmark/state.ts` — `initBenchmark`, `getBenchmarkPost`, `skipBenchmark`, `completeBenchmark`, `repairBenchmarkResults`, `repairSkipOptionalBenchmark`, `createBenchmarkRun`, `updateBenchmarkStatus`, `listBenchmarkRuns`, `getBenchmarkRequirement`
- `src/core/benchmark/results.ts` — input/canonical result parsing, `canonicalizeBenchmarkResults`, `writeResults`, `readResults`, `readBenchmarkSummary`, `writeEnvironment`, `readEnvironment`, slug validation
...
- `.claude/commands/evaluate.md`, `.claude/commands/plan-feature.md`, `.claude/commands/review.md`, `.agents/skills/source-command-evaluate/SKILL.md`, `.agents/skills/source-command-plan-feature/SKILL.md`, `.agents/skills/source-command-review/SKILL.md`, `.windsurf/workflows/review.md` — command guidance protected by `tests/guidance-docs.test.ts`
- `.claude/rules/drafting.md`, `.codex/rules/drafting.md`, `.claude/rules/lifecycle.md`, `.codex/rules/lifecycle.md`, `.claude/rules/release.md`, `.codex/rules/release.md`, `.codex/rules/testing.md` — cross-surface rule guidance for frontmatter, Dev.to API contracts, fallback keys, and review-suite registration
...
- `src/core/publish/table-assets.ts` — deterministic Markdown table detection, portable table PNG rendering, stale generated table cleanup, manifest metadata
- `src/core/publish/platform-copy.ts` — natural platform copy fitting, abrupt-ellipsis detection, and shared character budgets for LinkedIn/Substack copy surfaces
...
- `src/core/publish/platform-images.ts` — `ensurePlatformImages`, platform specs, safe asset resolver, local Sharp transforms, deterministic fallback SVG, receipt/frontmatter updates
...
- `templates/social/linkedin.md` — LinkedIn template with `{{hook}}`, `{{body}}`, `{{canonical_url}}`, `{{hashtags}}`, `{{timing}}` placeholders
...
- `.codex/rules/lifecycle.md` — path-scoped rule doc: publishMode dispatch, first-class update_cycles rows, partial-unique-index constraint, explicit isUpdateReview flag, cycle-keyed notice marker, publish guard, shared per-slug lock, shared finalize helper, metrics audit log, unpublish trust boundaries, origin-guard tolerant/strict APIs, readme-revert direct-push policy clarification
...
- `.claude/rules/voice.md` — dev-facing voice rule (path-scoped auto-load for draft/evaluate/skill files). Hard rules: no hedges, no tricolon stacks, no topic-restatement transitions, no smarmy openers, no undefined jargon, ~1 em dash per 500 words max with period/comma/parens/cut substitution test, no emojis. Two accessibility tests (newcomer + expert). Structural reviewer loads these hard rules and emits `category='voice'` issues when the draft drifts. Must stay byte-parity with `.claude-plugin/skills/blog/VOICE.md` (enforced by `tests/voice-parity.test.ts`).
- `.claude-plugin/skills/blog/VOICE.md` — plugin-shipped voice rule (no frontmatter; plugin skills don't path-scope). Loaded by `/blog` SKILL.md when composing `blog research set-section` prose so hash-bound section content is voice-compliant before entering the hash gate. Byte-identical to `.claude/rules/voice.md` body (parity test blocks drift).
- `src/core/draft/template.ts` — `renderDraftTemplate` prepends a `VOICE_COMMENT` JSX block between frontmatter and the first `##` section; MDX strips JSX comments at compile time so the comment survives as context for the draft author/agent but is invisible in the rendered hub output.
...
- `.github/PULL_REQUEST_TEMPLATE.md` — ≤10 line checklist with absolute GitHub URLs to AGENTS.md (relative paths 404 in GitHub's PR body rendering)
...
Expected baseline: **0 TypeScript errors, 1137 tests passing across 79 suites, clean build**. v0.3 dogfood-hardening added +43 tests across +5 new suites — `research-project-linking` (15), `publish-origin-divergence` (6), `publish-seed-on-start` (4), `preview-gate-urls` (9), `draft-regenerate-frontmatter` (9) — plus +8 tests in `draft-frontmatter.test.ts` for companion_repo resolution. Platform article images added Dev.to/Medium/Substack-preview coverage and expanded draft/crosspost coverage. Durable distribution kits add `distribution-kit.test.ts`, `publish-distribution-kit.test.ts`, `publish-table-assets.test.ts`, and CLI/config/plan-file coverage. Benchmark phase-gates, publish artifact verification, review-inventory tripwires, natural Substack subtitle fitting/failure, pre-clipped subtitle rejection, public-only LinkedIn copy, no-ellipsis HN copy, sanitized image prompts, bounded local-card SVG text, local-card LinkedIn image defaults, copyable chart paste output, portable table image assets, complete-bundle backfill, disabled-channel cleanup, paste-step table verification, stale deletion crash-replay coverage, Medium/Substack upload-checklist handoff, site/direct-push cleanup staging, mirrored Claude review inventory coverage, readable row-card table images with snake_case preservation, pre-clipped LinkedIn fallback copy, legacy prompt-only manifest loading, symlink-protected local-card writes, expected-hash manifest reuse, social-text tamper regeneration, and self-heal source-plan selection bring the current suite to 1137/79; any drift from this baseline is a signal to investigate before merging.
...
4. **Race conditions** or concurrency issues — especially relevant when CLI and Codex skills both write to `state.db`
...
9. **Pattern violations** — check AGENTS.md and `.codex/rules/` for project conventions:
...
Phase 3 — Benchmark:
  ...
  - Benchmark state lifecycle (17 tests): ✓ / ✗
  - Results storage (11 tests): ✓ / ✗
  - Companion repo scaffolding (6 tests): ✓ / ✗
  - Benchmark CLI handlers (19 tests): ✓ / ✗
  - Benchmark repair CLI (7 tests): ✓ / ✗

Phase 4 — Draft:
  - PostFrontmatter schema (32 tests): ✓ / ✗
  - Draft state lifecycle (17 tests): ✓ / ✗
  - Benchmark data formatting (9 tests): ✓ / ✗
  - Tag taxonomy reader (6 tests): ✓ / ✗
  - Draft CLI handlers (33 tests): ✓ / ✗
...
Phase 5 — Evaluate:
  ...
  - Evaluation state lifecycle (81 tests): ✓ / ✗
  - Evaluate CLI handlers (24 tests): ✓ / ✗

Phase 6 — Publish:
  - Phase + steps-crud + lock + crash-safety + reconcile (43 tests): ✓ / ✗
  - Pipeline runner + crash-safety + reconcile regressions (20 tests): ✓ / ✗
  - MDX → Markdown converter (15 tests): ✓ / ✗
  - Portable table asset rendering (11 tests): ✓ / ✗
  - Site PR + preview gate + dirty-state + rename/copy guardrail (21 tests): ✓ / ✗
  - Dev.to probe-then-create + Medium + Substack paste (40 tests): ✓ / ✗
  - Deterministic platform image generation (22 tests): ✓ / ✗
  - Social text: LinkedIn + Hacker News (17 tests): ✓ / ✗
  - Durable distribution-kit generation (28 tests): ✓ / ✗
  - Pre-site generation + strict distribution persistence (26 tests): ✓ / ✗
  - Research page generator (15 tests): ✓ / ✗
  - Companion repo probe-then-create + origin-URL guardrail (21 tests): ✓ / ✗
  - Publish CLI handlers + ctx.urls hydration + distribution-kit backfill (20 tests): ✓ / ✗
...
Phase 7 — Lifecycle:
  ...
  - Update CLI handlers (abort artifact preservation) (7 tests): ✓ / ✗
...
/blog Skill Plugin:
  ...
  - Plan schema + hash + validator + apply + receipt + lock (64 tests): ✓ / ✗
...
Full Suite: X passing, Y failing  (baseline: 1137 passing across 79 suites)
