---
name: "source-command-self-heal"
description: "Capture durable lessons from recent work into rules, tripwires, commands, and workflows"
---

# source-command-self-heal

Use this skill when the user asks to run the migrated source command
`self-heal`. Original argument hint: `[plan-file-path-or-feature-name]`.
...
If `{user-provided arguments}` is provided, use it as the plan file path or
feature name. If not provided, default to the most recently modified source plan
across Claude and Codex plan folders. Prefer a plan with a `## Contract`
section, because proof/evidence notes can also live under `plans/` and are not
the source material for a retrospective.
