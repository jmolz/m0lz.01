...
`PostFrontmatter` in `src/core/draft/frontmatter.ts` has exactly these fields: `title`, `description`, `date`, `tags`, `published`, and optional `canonical`, `companion_repo`, `project`, `medium_url`, `substack_url`, `devto_url`, `devto_main_image`, `medium_featured_image`, `substack_preview_image`, `substack_header_image`, `unpublished_at`, `updated_at`, and `update_count`. These mirror the schema that m0lz.00 consumes.
