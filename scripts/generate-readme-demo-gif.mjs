...
const OUTPUT = resolve('readme-quickstart.gif');
const FRAME_DIR = mkdtempSync(join(tmpdir(), 'm0lz-readme-demo-'));

const lines = [
  { text: '$ npm install -g m0lz-01', kind: 'cmd' },
  { text: 'm0lz-01 installed', kind: 'out' },
  { text: '$ mkdir -p ~/blog && cd ~/blog', kind: 'cmd' },
  { text: '$ blog init', kind: 'cmd' },
  { text: 'created .blog-agent/state.db', kind: 'ok' },
  { text: 'copied .blogrc.yaml and .env', kind: 'ok' },
  { text: '$ blog research init m0lz-01-launch --topic "m0lz.01 launch"', kind: 'cmd' },
  { text: 'Created research entry for m0lz-01-launch', kind: 'ok' },
  { text: '$ blog research add-source m0lz-01-launch --url https://m0lz.dev', kind: 'cmd' },
  { text: 'Added source (id=1)', kind: 'ok' },
  { text: '$ blog status', kind: 'cmd' },
  { text: 'm0lz-01-launch    research    1 source tracked', kind: 'out' },
];
...
<text x="64" y="104" fill="#f5f5f0" font-family="Inter, Arial, sans-serif" font-size="27" font-weight="700">m0lz.01 quickstart</text>
