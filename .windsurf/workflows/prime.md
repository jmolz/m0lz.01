...
**If HANDOFF.md exists**, cross-reference its open items against `git log` to verify they're still relevant. Remove or mark completed any items that git history shows have been addressed. If all items are resolved, note that the handoff is fully resolved and can be deleted.

### 6. Check Windsurf Rules and Workflows

Read the Windsurf rules to understand project conventions:

- `.windsurf/rules/global.md` — always-on global rules
- `.windsurf/rules/cli.md` — CLI handler patterns
- `.windsurf/rules/database.md` — SQLite conventions
- `.windsurf/rules/testing.md` — Vitest patterns

Scan available workflows:

```bash
ls .windsurf/workflows/*.md 2>/dev/null
```
