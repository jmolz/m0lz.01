...
| `tests/config.test.ts` (17 tests) | `.blogrc.yaml` loader | Valid config parses with repo_path resolved relative to config dir; missing required fields throw descriptive errors; optional sections get sensible defaults; non-existent config file throws; distribution-kit defaults to local-card LinkedIn image generation; explicit prompt-only compatibility and invalid modes are covered |
...
| `tests/publish-convert.test.ts` (15 tests) | MDX → Markdown converter | Strips frontmatter, imports, JSX self-closing tags; converts image-backed JSX chart/figure components to copyable Markdown images; preserves text children of block JSX; resolves relative asset URLs to absolute `{base}/writing/{slug}/assets/x`; preserves code blocks verbatim (no JSX stripping / no URL resolution inside ``` fences, including nested backtick fences); preserves tables/headings/lists; collapses 3+ blank lines to 2 |
...
| `tests/publish-site.test.ts` (21 tests) | Site PR + preview gate + dirty-state + rename/copy guardrail | `createSitePR` copies MDX + assets + research page to site repo; copies complete publication bundle artifacts (`distribution/manifest.json`, social text, Medium/Substack paste files, portable table images) before preview; branch `post/{slug}` idempotent; `feat(post):` commit; `gh pr create` idempotent on existing PR; `pr-number.txt` written for preview-gate; dirty-state guardrail throws on uncommitted unrelated changes; tolerates dirty state only on exact pipeline-owned paths; stages exact current outputs plus tracked stale owned deletions; recomputes branch-dependent stale deletion staging after existing-branch checkout; rename/copy porcelain parsing rejects R/C escapes; `checkPreviewGate` polls gh pr view; all subprocess via execFileSync argv arrays |
| `tests/publish-crosspost.test.ts` (40 tests) | Dev.to probe-then-create + Medium + Substack paste | `mapDevToTags` lowercases, strips non-alphanumerics for Dev.to alphanumeric-only tags, caps at 4, drops empty, maps dashed source tags like `developer-tools` -> `developertools`; `crosspostToDevTo` skips when DEVTO_API_KEY unset; builds POST with api-key header, canonical_url, unpublished status, tags, description, optional safe cover image; throws on unsafe cover paths, 422, and 503+; probe-then-converge GETs `/api/articles/me/all`, PUTs existing unpublished canonical matches, rejects duplicate/published/unknown-state matches, normalizes trailing slash, fails closed on probe errors, paginates, and stops on a short page; Medium/Substack paste keeps image-backed MDX charts copyable, preserves code blocks, naturally fits Substack subtitles without ellipses, replaces already-clipped subtitles with safe fallback copy, and fails when no safe fit exists, replaces raw Markdown tables outside fences with upload-checklist placeholders, and writes platform-specific table upload checklists with canonical URL, local paths, reference-only public URLs, alt text, and captions |
...
| `tests/publish-cli.test.ts` (20 tests) | Publish CLI handlers + ctx.urls hydration + distribution-kit backfill | Invalid slug → exitCode=1 before DB open; wrong phase → valid-phases hint; evaluate → promote + seed 11 + runPipeline; publish phase → resume; pausedStep → info (not error); failedStep → exitCode=1; runner throws → "pipeline crashed"; runPublishShow table + empty-state; malformed config best-effort; ctx.urls hydrated from posts row before runPipeline; fresh publish hydrates to empty object; `blog publish distribution-kit` backfills local artifacts for published posts; published posts can regenerate platform images; reopen-draft handles failed pre-site recovery |
...
| `tests/update-cli.test.ts` (7 tests) | `blog update abort/start/show` | abort closes cycle + writes metric + PRESERVES `.blog-agent/drafts/{slug}/` artifacts; exit 1 when no open cycle; invalid slug exit 1 before DB open; start refuses empty --summary when require_summary=true; show prints phase/update_count/cycle table |
...
| `tests/build-bin-executable.test.ts` (1 test) | Build preserves +x on CLI entrypoint | `tsc` strips owner-execute on every rebuild; `scripts/chmod-bin.mjs` runs as the last step of `npm run build`; test asserts `statSync(dist/cli/index.js).mode & 0o100 === 0o100` after vitest globalSetup runs the build. Catches any future refactor that drops the chmod step before the next contributor hits "Permission denied: blog" inside a `/blog` skill exec fence. |
| `tests/research-set-section.test.ts` (13 tests) | `blog research set-section` CLI + library | `setResearchSection` replaces one of 7 keyed sections (thesis, findings, sources_list, data_points, open_questions, benchmark_targets, repo_scope) while preserving frontmatter + other sections; invalid section key enumerated in error; path-traversal slug rejected; missing-doc actionable hint; overwrite semantics (replace, not append); all-7 satisfies doc-gate half of finalize. CLI: `--content` inline path; `--from-file <path>`; mutual-exclusion; empty-content rejected; non-research phase rejected; missing file rejected; path-traversal slug rejected at CLI before DB open. Enables skill-driven research authoring — hash-bound section prose travels through the plan, no Write/Edit scope needed in the skill. |
| `tests/voice-parity.test.ts` (4 tests) | Voice rules byte-parity (dev rule ↔ plugin skill) | `.claude/rules/voice.md` (dev auto-load) and `.claude-plugin/skills/blog/VOICE.md` (plugin tarball) must stay byte-identical below the frontmatter boundary. `stripFrontmatter` extracts body after closing `---\n`; parity assertion catches drift. Dev rule must have `paths:` frontmatter; plugin file must have NO frontmatter; plugin file must not contain `jmolz | m0lz.dev | @molz | DEVTO_API_KEY=` identity strings. Prevents "voice rule updated in one place, other location drifts" silent divergence. |
...
- `src/cli/benchmark.ts` — includes `runBenchmarkRepair`
...
- `.claude/commands/evaluate.md`, `.claude/commands/plan-feature.md`, `.claude/commands/review.md`, `.agents/skills/source-command-evaluate/SKILL.md`, `.agents/skills/source-command-plan-feature/SKILL.md`, `.agents/skills/source-command-review/SKILL.md`, `.windsurf/workflows/review.md`
- `.claude/rules/drafting.md`, `.codex/rules/drafting.md`, `.claude/rules/lifecycle.md`, `.codex/rules/lifecycle.md`, `.claude/rules/release.md`, `.codex/rules/release.md`, `.codex/rules/testing.md`
...
- `src/cli/research.ts` — adds `runResearchSetSection(slug, opts)` with mutual-exclusive `--content` / `--from-file`, empty-content rejection, non-research-phase rejection, CLI-boundary slug validation.
- `src/core/draft/template.ts` — `renderDraftTemplate` prepends a `VOICE_COMMENT` JSX block between frontmatter and the first `##` section; MDX strips JSX comments at compile time so the comment is invisible in the rendered hub output while surviving as context for the draft author/agent.
...
Phase 3 — Benchmark:
  ...
  - Benchmark state lifecycle (17 tests): PASS / FAIL
  - Results storage (11 tests): PASS / FAIL
  - Companion repo scaffolding (6 tests): PASS / FAIL
  - Benchmark CLI handlers (19 tests): PASS / FAIL
  - Benchmark repair CLI (7 tests): PASS / FAIL

Phase 4 — Draft:
  - PostFrontmatter schema (32 tests): PASS / FAIL
  - Draft state lifecycle (17 tests): PASS / FAIL
  - Benchmark data formatting (9 tests): PASS / FAIL
  - Tag taxonomy reader (6 tests): PASS / FAIL
  - Draft CLI handlers (33 tests): PASS / FAIL
...
Phase 5 — Evaluate:
  ...
  - Evaluation state lifecycle (81 tests): PASS / FAIL
  - Evaluate CLI handlers (24 tests): PASS / FAIL
...
Phase 6 — Publish:
  ...
  - MDX → Markdown converter (15 tests): PASS / FAIL
  - Portable table asset rendering (11 tests): PASS / FAIL
  - Site PR + preview gate + dirty-state + rename/copy guardrail (21 tests): PASS / FAIL
  - Dev.to probe-then-create + Medium + Substack paste (40 tests): PASS / FAIL
  - Deterministic platform image generation (22 tests): PASS / FAIL
  - Social text: LinkedIn + Hacker News (17 tests): PASS / FAIL
  - Durable distribution-kit generation (28 tests): PASS / FAIL
  - Pre-site generation + strict distribution persistence (26 tests): PASS / FAIL
  - Research page generator (15 tests): PASS / FAIL
  - Companion repo probe-then-create + origin-URL guardrail (21 tests): PASS / FAIL
  - Publish CLI handlers + ctx.urls hydration + distribution-kit backfill (20 tests): PASS / FAIL
...
Phase 7 — Lifecycle:
  ...
  - Update CLI handlers (abort artifact preservation) (7 tests): PASS / FAIL
...
/blog Skill Plugin:
  ...
  - Plan schema + hash + validator + apply + receipt + lock (64 tests): PASS / FAIL
