...
| `tests/distribution-kit.test.ts` (8 tests) | Durable distribution-kit generation | Metadata precedence by publish/update/backfill; prompt-only writes text/prompt/manifest without provider calls; generated mode records GPT Image 2 model/size/quality/mode/fixed PNG format, prompt/input/text/image hashes, byte size, dimensions, and reuses matching manifests byte-for-byte; `loadDistributionKit` refuses tampered text and image artifacts whose bytes no longer match manifest hashes; required mode fails without `OPENAI_API_KEY`; product IDs like `m0lz.01` are not truncated |
...
Phase 6 — Publish:
  ...
  - Durable distribution-kit generation (8 tests): PASS / FAIL
