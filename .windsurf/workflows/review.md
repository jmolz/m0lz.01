...
| `tests/publish-crosspost.test.ts` (38 tests) | Dev.to probe-then-create + Medium + Substack paste | `mapDevToTags` lowercases, strips non-alphanumerics for Dev.to alphanumeric-only tags, caps at 4, drops empty, maps dashed source tags like `developer-tools` -> `developertools`; `crosspostToDevTo` skips when DEVTO_API_KEY unset; builds POST with api-key header, canonical_url, unpublished status, tags, description, optional safe cover image; throws on unsafe cover paths, 422, and 503+; probe-then-converge GETs `/api/articles/me/all`, PUTs existing unpublished canonical matches, rejects duplicate/published/unknown-state matches, normalizes trailing slash, fails closed on probe errors, paginates, and stops on a short page; Medium/Substack paste keeps image-backed MDX charts copyable, preserves code blocks, caps Substack subtitles, and replaces raw Markdown tables outside fences with manifest-backed portable table image links |
...
Phase 6 — Publish:
  ...
  - Portable table asset rendering (6 tests): PASS / FAIL
