...
| `tests/config.test.ts` (15 tests) | `.blogrc.yaml` loader | Valid config parses with repo_path resolved relative to config dir; missing required fields throw descriptive errors; optional sections get sensible defaults; non-existent config file throws; distribution-kit and LinkedIn image defaults/overrides validate |
...
| `tests/publish-crosspost.test.ts` (38 tests) | Dev.to probe-then-create + Medium + Substack paste | `mapDevToTags` lowercases, strips non-alphanumerics for Dev.to alphanumeric-only tags, caps at 4, drops empty, maps dashed source tags like `developer-tools` -> `developertools`; `crosspostToDevTo` skips when DEVTO_API_KEY unset; builds POST with api-key header, canonical_url, unpublished status, tags, description, optional safe cover image; throws on unsafe cover paths, 422, and 503+; probe-then-converge GETs `/api/articles/me/all`, PUTs existing unpublished canonical matches, rejects duplicate/published/unknown-state matches, normalizes trailing slash, fails closed on probe errors, paginates, and stops on a short page; Medium/Substack paste keeps image-backed MDX charts copyable, preserves code blocks, caps Substack subtitles, replaces raw Markdown tables outside fences with upload-checklist placeholders, and writes platform-specific table upload checklists with canonical URL, local paths, reference-only public URLs, alt text, and captions |
...
| `tests/publish-social.test.ts` (14 tests) | LinkedIn + Hacker News text generation | `containsEmoji` detects emoji ranges; `generateLinkedIn` fills template with title, description, image/prompt path, alt text, hashtags, canonical URL, throws on emojis in output, includes timing when configured; `generateHackerNews` title ≤80 chars (truncate with ellipsis); `Show HN:` prefix ONLY for project-launch; includes first-comment, canonical_url, real repo link only; throws on emoji; writes linkedin.md/hackernews.md |
| `tests/distribution-kit.test.ts` (19 tests) | Durable distribution-kit generation | Metadata precedence by publish/update/backfill; hub MDX/frontmatter wins for published backfill; prompt-only writes text/prompt/manifest without provider calls; Medium/Substack paste files, upload checklists, and portable table assets are manifest-tracked and hash-verified; social-disabled configs still emit paste artifacts when cross-post platforms are enabled; disabled platforms stay null per individual destination; generated mode records GPT Image 2 provenance and reuses matching manifests byte-for-byte; `loadDistributionKit` refuses tampered text/checklist/image/table bytes, unsafe manifest paths, symlinks, and unsafe reused manifests; required mode fails without `OPENAI_API_KEY`; product IDs like `m0lz.01` are not truncated |
| `tests/publish-distribution-kit.test.ts` (25 tests) | Pre-site generation + strict persistence | Required LinkedIn image mode fails before site checkout/copy/commit/push/PR when the OpenAI key is missing; the real `social-text` registry step persists an existing manifest-derived kit including Medium/Substack paste files, upload checklists, table assets, and optional `assets/linkedin-feed.png` without provider/fetch calls; paste steps verify existing manifest artifacts without regenerating paste/table assets; disabled paste steps skip without requiring bundle files; direct site persistence stages only owned distribution paths and exact optional assets, commits `chore(distribution): <slug>`, removes tracked stale generator-owned text, LinkedIn image, and portable-table artifacts when channels/images/tables are disabled or regenerated, enforces origin and staged-index guards, refuses unrelated dirty site state, conflicting reviewed artifacts, and unexpected ahead commits before staging, returns exact no-op when current, pushes exact crash-replay ahead commits including already-committed stale artifact deletions, and refuses wrong-subject/wrong-file ahead commits |
...
Phase 6 — Publish:
  ...
  - Portable table asset rendering (10 tests): PASS / FAIL
  - Site PR + preview gate + dirty-state + rename/copy guardrail (21 tests): PASS / FAIL
  - Dev.to probe-then-create + Medium + Substack paste (38 tests): PASS / FAIL
  - Deterministic platform image generation (22 tests): PASS / FAIL
  - Social text: LinkedIn + Hacker News (14 tests): PASS / FAIL
  - Durable distribution-kit generation (19 tests): PASS / FAIL
  - Pre-site generation + strict distribution persistence (25 tests): PASS / FAIL
  - Research page generator (14 tests): PASS / FAIL
