...
| `tests/distribution-kit.test.ts` (9 tests) | Durable distribution-kit generation | Metadata precedence by publish/update/backfill; prompt-only writes text/prompt/manifest without provider calls; generated mode records GPT Image 2 model/size/quality/mode/fixed PNG format, prompt/input/text/image hashes, byte size, dimensions, and reuses matching manifests byte-for-byte; `loadDistributionKit` refuses tampered text and image artifacts whose bytes no longer match manifest hashes; `OpenAIImageProvider` omits unsupported `response_format`; required mode fails without `OPENAI_API_KEY`; product IDs like `m0lz.01` are not truncated |
| `tests/publish-distribution-kit.test.ts` (10 tests) | Pre-site generation + strict persistence | Required LinkedIn image mode fails before site checkout/copy/commit/push/PR when the OpenAI key is missing; the real `social-text` registry step persists an existing kit plus optional `assets/linkedin-feed.png` without provider/fetch calls; direct site persistence stages only distribution paths, commits `chore(distribution): <slug>`, enforces origin and staged-index guards, refuses unrelated dirty site state and unexpected ahead commits before staging new distribution changes, returns exact no-op when current, pushes exact crash-replay ahead commits, and refuses wrong-subject or wrong-file ahead commits |
...
Phase 6 — Publish:
  ...
  - Dev.to probe-then-create + Medium + Substack paste (33 tests): PASS / FAIL
  - Deterministic platform image generation (21 tests): PASS / FAIL
  - Social text: LinkedIn + Hacker News (14 tests): PASS / FAIL
  - Durable distribution-kit generation (9 tests): PASS / FAIL
  - Pre-site generation + strict distribution persistence (10 tests): PASS / FAIL
  - Research page generator (14 tests): PASS / FAIL
  - Companion repo probe-then-create + origin-URL guardrail (21 tests): PASS / FAIL
  - Publish CLI handlers + ctx.urls hydration + distribution-kit backfill (16 tests): PASS / FAIL
