...
This is critical — the next session needs to know where to resume work.

### 4. Write HANDOFF.md

Save to `HANDOFF.md` in the project root (the main repo, not the worktree). Use the template at `.claude/templates/HANDOFF-template.md` if it exists — fill in every section with real data from reconciliation + current session.
