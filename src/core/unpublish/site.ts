...
import { expectedSiteCoords, requireOriginMatch } from '../publish/origin-guard.js';
...
export function createSiteRevertPR(
  ...
  // Expected site repo target comes from config.site.github_repo when
  // set, else `{author.github}/basename(repo_path)`. Strict variant:
  // unpublish never creates origin, so "no origin" must fail loudly.
