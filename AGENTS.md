...
m0lz.01 is a local idea-to-distribution pipeline that orchestrates the full lifecycle of technical content: research, benchmark, draft, evaluate, publish, and distribute. It publishes to [m0lz.dev](https://m0lz.dev) (m0lz.00) as the canonical hub, with cross-post platforms and project repos as spokes. The agent runs locally using Codex for interactive AI work and a standalone CLI for mechanical operations.
...
**Standalone CLI** (mechanical, no AI needed): `blog init`, `blog publish`, `blog unpublish`, `blog status`, `blog metrics`, `blog ideas`, `blog research init|add-source|show|set-section|finalize`, `blog benchmark init|env|run|show|skip|complete|repair`, `blog draft init|show|validate|add-asset|complete|platform-images|regenerate-frontmatter`, `blog evaluate init|structural-autocheck|record|show|synthesize|complete|reject`, `blog publish distribution-kit|platform-images` — these run independently for API calls, state queries, and pipeline execution.
...
| Reviewer | Model | Role |
|----------|-------|------|
| Structural | Codex | Content quality, schema, MDX, sources |
...
Both Codex reviewers have an OpenAI Responses API fallback via `~/.codex/.openai-fallback-key` — activates on Codex rate-limit, crash, auth-miss, or empty output. The fallback labels reviewer JSON with `gpt-5.5-<effort>-fallback-api` so synthesis audits distinguish the paths. Full pattern at `skills/blog-evaluate.md` § Codex failure handling.
...
| File | Scope | Covers |
|------|-------|--------|
| `.codex/rules/database.md` | schema/DB files | SQLite conventions, WAL, foreign keys, migrations, type mapping |
| `.codex/rules/cli.md` | `src/cli/**` | Handler shape, injectable paths, exit codes, DB cleanup, error boundary, best-effort config for show commands |
| `.codex/rules/testing.md` | `tests/**`, `*.test.ts` | Temp dirs, `:memory:` DBs, mocking console/exit, fixtures, regression suite |
| `.codex/rules/drafting.md` | `src/core/draft/**`, `src/cli/draft.ts`, `templates/draft/**` | PostFrontmatter contract, MDX parsing, placeholder tokens, content-type routing, asset safety |
...
| `.codex/rules/publish.md` | `src/core/publish/**`, `src/cli/publish.ts` | Evaluated draft immutability during site-pr/site-update, platform image reference modes, deterministic config-derived image generation |
...
| Area | File | When |
|------|------|------|
| Full product spec | `.codex/PRD.md` | Understanding scope, features, architecture |
| Phase 1 plan | `.codex/plans/phase-1-foundation.md` | Implementing foundation |
| Phase 2 plan | `.codex/plans/phase-2-research.md` | Research pipeline + contract |
| Phase 3 plan | `.codex/plans/phase-3-benchmark.md` | Benchmark test harness + contract |
| Phase 4 plan | `.codex/plans/phase-4-draft.md` | Draft + visuals + contract |
| Phase 5 plan | `.codex/plans/phase-5-evaluate.md` | Three-reviewer adversarial evaluation + contract |
| Phase 6 plan | `.codex/plans/phase-6-publish.md` | Initial-publish 11-step pipeline + contract |
| Phase 7 plan | `.codex/plans/phase-7-lifecycle.md` | Unpublish + update + orchestration + contract |
| Drafting rules | `.codex/rules/drafting.md` | MDX parsing pitfalls, PostFrontmatter contract, placeholder tokens |
| Publish rules | `.codex/rules/publish.md` | Site PR/update draft immutability, platform image reference handling |
| Original brainstorm | `.codex/plans/blog-agent-prd.md` | Historical context |
| PICE workflow | `.codex/docs/PLAYBOOK.md` | Plan/Implement/Evaluate loop |
| Agent teams | `.codex/docs/AGENT-TEAMS-PLAYBOOK.md` | Parallel agent coordination |
